...
.LC1:
	...
	printifm("\t.global __aeabi_idiv")
	printifm("\t.global __aeabi_idivmod")
...
def gen_expression(expression):
	if type(expression) == arbre_abstrait.Operation:
		...
		arm_instruction("mov", "r1", "#"+str(expression.valeur), "", "") # on met la valeur entière dans r1
		arm_instruction("push", "{r1}", "", "", "") # on empile la valeur
	else:
...
def gen_operation(operation):
	...
	#Voir: https://developer.arm.com/documentation/dui0497/a/the-cortex-m0-instruction-set/instruction-set-summary?lang=en
		...
		arm_instruction("bl", "__aeabi_idiv", "", "", "appel à la division entière r0 / r1 → r0")
		arm_instruction("mov", "r1", "r0", "", "copie résultat dans r1")
	elif op == '%':
		arm_instruction("bl", "__aeabi_idivmod", "", "", "r1=r0 % r1")
